...
ALIGN(RT_ALIGN_SIZE)
static uint8_t rx_thread_stack[CONFIG_BT_RX_STACK_SIZE];
static struct rt_thread rx_thread_data;
#endif

ALIGN(RT_ALIGN_SIZE)
static uint8_t tx_thread_stack[CONFIG_BT_HCI_TX_STACK_SIZE];
static struct rt_thread tx_thread_data;

static void init_work(struct k_work *work, void *work_data);
...
/**
 * @brief Init bt_dev, ported by Jackistang.
*/
...
static void rpa_timeout(struct k_work *work, void *work_data)
{
	bool adv_enabled = false;
...
int bt_enable(bt_ready_cb_t cb)
{
	...
	/* TX thread */
	rt_thread_init(&tx_thread_data, "BT TX", hci_tx_thread, NULL, 
				tx_thread_stack, ARRAY_SIZE(tx_thread_stack),
				CONFIG_BT_HCI_TX_PRIO, 10);
	rt_thread_startup(&tx_thread_data);
...
#if !defined(CONFIG_BT_RECV_IS_RX_THREAD)
	/* RX thread */
	rt_thread_init(&rx_thread_data, "BT RX", hci_rx_thread, NULL, 
				rx_thread_stack, ARRAY_SIZE(rx_thread_stack),
				CONFIG_BT_RX_PRIO, 10);
	rt_thread_startup(&rx_thread_data);
